...
const CRISIS_KEYWORDS = [
  ...
  'not worth living','give up on life','ending it all','overdose'
];

const NEGATIONS = ['not','no',"isn't","aren't","wasn't","weren't","don't","doesn't","didn't","can't","couldn't","won't","wouldn't","never","neither","nor"];

const POSITIVE_WORDS = ['good','great','happy','joy','love','excited','wonderful','amazing','fantastic','grateful','awesome','positive','calm','peaceful','relaxed','content','fine','okay','better','best','perfect','excellent','cheerful','optimistic','hopeful'];
const NEGATIVE_WORDS = ['bad','sad','depress','anxi','stress','worr','tired','exhaust','angry','frustrat','lonely','scared','afraid','hopeless','worthless','overwhelm','fail','hate','awful','terrible','horrible','miserable','upset','disappoint','hurt','pain','cry','crying','lost','broken','empty','weak','struggle','difficult','hard','sick','numb'];
...
function analyzeText(text) {
  ...
  words.forEach((word, i) => {
    ...
    const negated = prevWords.some(p => NEGATIONS.includes(p));

    const isPositive = POSITIVE_WORDS.some(p => word.startsWith(p));
    const isNegative = NEGATIVE_WORDS.some(n => word.startsWith(n));

    if (isPositive) score += negated ? -1.5 : 1;
    if (isNegative) score += negated ? 0.5 : -1.5;
  });

  // Detect full negation phrases explicitly
  if (lower.includes('not good') || lower.includes('not okay') || lower.includes("im not okay") || lower.includes('not fine') || lower.includes('not well') || lower.includes('not great') || lower.includes("i'm not good")) score -= 2;
  if (lower.includes('not bad') || lower.includes('not terrible')) score += 1;
  ...
  else if (comparative >= 0.4) emotion = 'joyful';
  else if (comparative >= 0.15) emotion = 'happy';
  else if (comparative >= 0.03) emotion = 'calm';
...
const RESPONSES = {
  ...
  concerned: ["I sense something might be weighing on you. 💙 Would you like to talk about what's on your mind?","It sounds like things are a bit challenging. It's okay not to be okay — I'm here to listen.","Your feelings are valid. 🤝 Would it help to talk through what's been going on?"],
  sad: ["I'm sorry you're going through a difficult time. 💙 Your feelings are completely valid. Would you like to talk?","It takes courage to acknowledge when we're not okay. I'm right here with you. 🤗","You don't have to carry this alone. What would feel most helpful right now — talking it out, or a calming exercise?"],
  anxious: ["Anxiety can feel overwhelming, but you've taken a brave step by reaching out. 💙 Try naming 5 things you can see right now.","Let's try 4-7-8 breathing together: breathe in for 4 counts, hold for 7, exhale for 8. 🫁","What's triggering your anxiety? Sometimes naming it takes away some of its power."],
  distressed: ["I can tell you're going through something really tough right now. I want you to know you are not alone. 💙","Your pain is valid. Your safety matters most right now. Please reach out to someone you trust or a helpline.","What you're feeling sounds really heavy, and no one should have to face that alone. Let's talk about getting you some support. 💙"]
};
...
const STRATEGIES = {
  ...
  distressed: [{ title: 'Immediate Grounding', description: 'Splash cold water on your face or press your feet firmly on the ground.' }, { title: 'Reach Out Now', description: 'iCall India: 9152987821 | Vandrevala: 1860-2662-345 | Crisis Text Line: text HOME to 741741.' }]
};
