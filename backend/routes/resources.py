"""
Self-help resources and crisis information routes.
Serves curated static content (same approach as the Node.js backend).
"""
...
@router.get("/")
def get_resources(category: str = Query(default=None)):
    """Get all self-help resources, optionally filtered by category."""
    if category:
        found = next((c for c in RESOURCES["categories"] if c["id"] == category), None)
        if not found:
...
def get_crisis_resources():
    """Get crisis/emergency resources only."""
    return {"success": True, "data": RESOURCES["crisisResources"]}
